...
def chat():
    try:
        ...
        # --- 1. SPESIFIK LOKER: GAJI / UPAH / BAYARAN ---
        if any(k in msg for k in ['gaji', 'upah', 'bayaran', 'insentif', 'take home pay']):
            reply = (
                f"Halo Kak! 😊 Untuk informasi mengenai **gaji/upah** dan detail hak karyawan di {ctx.get('nama_toko', 'Toko Buah ABS Kepanjen')}, "
                f"dapat didiskusikan secara langsung saat tahap wawancara atau saat penyerahan berkas lamaran ke toko yaa.\n\n"
                f"🏠 **Fasilitas Tambahan:** {jam_fasilitas.get('fasilitas', 'Makan & minum disediakan di mess')}\n"
                f"🕒 **Jam Kerja:** {jam_fasilitas.get('jam_kerja', '07.00 - 21.00 WIB')}\n\n"
                f"Silakan kirimkan CV/lamaran Kakak terlebih dahulu! 🍎"
            )
            return jsonify({'response': reply})

        # --- 2. SPESIFIK LOKER: PERSYARATAN & SYARAT ---
        elif any(k in msg for k in ['syarat', 'persyaratan', 'kualifikasi', 'kriteria', 'umur', 'usia', 'cewek', 'perempuan']):
        ...
        # --- 3. SPESIFIK LOKER: JAM KERJA, LIBUR, MESS & FASILITAS ---
        ...
        # --- 4. SPESIFIK LOKER: BERKAS, CARA MELAMAR & PANGGILAN ---
        ...
        # --- 5. SPESIFIK LOKER: UMUM / STATUS LOKER ---
            ...
            reply = (
                ...
                f"👥 Kuota: {loker.get('kuota', '-')}\n\n"
                f"Kakak bisa tanyakan lebih spesifik mengenai *syarat*, *gaji*, *berkas lamaran*, atau *jam kerja & fasilitas* yaa! 🍎"
            )
            return jsonify({'response': reply})
        ...
        # --- 10. JAWABAN DEFAULT (RESPON SOPAN) ---
        else:
